...
@Component({
  selector: 'app-acertar-estoque',
  templateUrl: './acertar-estoque.component.html',
  styleUrls: ['./acertar-estoque.component.scss'],
  providers: [MessageService]
})
export class AcertarEstoqueComponent implements OnInit {
  ...
  acertarEstoque() {
    if (this.formGroup.valid && this.formGroup.value) {
      const values = {
        id_racao: +this.idRacao,
        quantidade: this.formGroup.get('quantidade')?.value
      }
      this.racaoService.acertarEstoqueRacao(values).subscribe({
        next: (res) => {
          console.log('Acerto de Estoque => ', res);
          this.formGroup.reset();
          this.messageService.add({ severity: 'success', summary: 'Estoque Acertado', detail: 'Estoque acertado com sucesso!' });
          this.getRacoes();
          setTimeout(() => {
            this.visible = false;
          }, 2000)
        ...
        error: (err) => {
          this.messageService.add({ severity: 'error', summary: 'Erro', detail: err.error.message || `Ocorreu um erro ao acertar o estoque` });
