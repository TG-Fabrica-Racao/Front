...
import { LogAcertos } from 'src/app/shared/models/racao';
import { RacaoService } from 'src/app/shared/services/racao.service';

@Component({
  selector: 'app-acertos-estoque',
  templateUrl: './acertos-estoque.component.html',
  styleUrls: ['./acertos-estoque.component.scss']
})
export class AcertosEstoqueComponent implements OnInit {
  currentPage: number = 1;
  itemsPerPage: number = 6;
  totalPages: number = 0;
  displayItems: LogAcertos[] = [];
  logs: LogAcertos[] = [];
  formGroup: FormGroup;
  ...
  getLogs() {
    this.racaoService.getAcertosLogs(this.formGroup.value).subscribe({
      next: (res) => {
